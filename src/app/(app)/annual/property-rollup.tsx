...
function gridStyle(monthCount: number) {
  return {
    gridTemplateColumns: `11rem minmax(6rem,1fr) repeat(${monthCount},minmax(4.5rem,1fr))`,
  };
}
function trackMinWidth(monthCount: number) {
  return { minWidth: `${11 + 6 + 4.5 * monthCount}rem` };
}
    ...
    <div className="overflow-clip rounded-lg bg-surface ring-1 ring-black/5 dark:ring-white/10">
      ...
      >
        ...
        <span
          ...
          aria-hidden
        />
        <span className="text-[13px] font-bold">{property.name}</span>
        {property.subtype ? (
          <span className="rounded-full bg-black/5 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted dark:bg-white/10">
            {property.subtype}
          </span>
        ) : null}
        ...
        empty ? (
          ...
          <div className="scroll-handle overflow-x-auto border-t border-line">
            <div style={trackMinWidth(monthCount)}>
              ...
              >
                <span className="sticky left-0 z-10 bg-surface pl-3 text-[10px] font-medium uppercase tracking-wide text-muted">
                  Line
                </span>
              ...
              <div
                ...
                style={gridStyle(monthCount)}
              >
                <span className="sticky left-0 z-10 bg-background pl-3 text-[13px] font-bold">Net</span>
                <span
...
function SummaryRow({
  ...
  return (
    <div className="grid items-center gap-2 py-2 pr-3" style={gridStyle(monthCount)}>
      <span
        className="sticky left-0 z-10 bg-surface pl-3 text-[12px] font-bold uppercase tracking-wide"
        style={{ color }}
      >
        {label}
      </span>
      <span className="text-center text-[11px] font-bold tabular-nums" style={{ color }}>
        {total !== 0 ? formatMoney(total, currency) : <span className="text-muted">—</span>}
      </span>
      {months.slice(0, monthCount).map((v, i) => (
        <span key={i} className="text-center text-[11px] font-semibold tabular-nums">
          {v !== 0 ? formatMoney(v, currency) : <span className="text-muted">—</span>}
        </span>
      ))}
    </div>
...
function LineRow({
  ...
  return (
    <div className="grid items-center gap-2 py-1.5 pr-3" style={gridStyle(monthCount)}>
      <span className="sticky left-0 z-10 truncate bg-surface pl-6 text-[12px] text-muted">
        {line.name}
      </span>
      ...
      {line.months.slice(0, monthCount).map((v, i) => (
        <span key={i} className="text-center text-[11px] tabular-nums text-muted">
...
function Figure({
  ...
  return (
    <span className="flex items-baseline gap-1">
      ...
      <span className={`text-[12px] ${strong ? "font-bold" : "font-semibold"}`} style={{ color }}>
