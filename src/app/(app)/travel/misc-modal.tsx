...
import { bookingTotalsFor } from "./trip-summary";
...
export function MiscModal({
  ...
  const bookedRows: { label: string; planned: number; actual: number }[] = [
    { label: "Stays", ...booked.stay },
    { label: "Flights", ...booked.flight },
    { label: "Rental", ...booked.car },
  ];
            ...
            {bookedRows.map((b) => (
              ...
              >
                ...
                <span className="col-span-2 flex items-baseline justify-between text-sm font-semibold sm:col-span-1 sm:block sm:text-center">
                  ...
                  {/* Nothing to compare against when the booking never was
                      an estimate — a dash, not a red "over by the full price". */}
                  {diffCell(b.planned ? b.planned - b.actual : null)}
                </span>
              </li>
            ))}
            {rows.map((r) => {
